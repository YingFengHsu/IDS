...
class IntrusionDetectionEnv(gym.Env):
    ...
    def _step(self, action_index):
        ...
        if self.turns < 50 :
            self.ob = np.delete(self.df0[self.n0 + self.turns], -38)
        else:
            self.ob = np.delete(self.df1[self.n1 + self.turns - 50], -38)
    ...
    def _reset(self):
        ...
        """
        self.n0, self.n1 = self._get_random_initial_state()
        self.ob = np.delete(self.df0[self.n0], -38)
        self.episode_over = False
        self.turns = 0
        #self.sum_rewards = 0.0
        return self.ob
    ...
    def _get_random_initial_state(self):
        n0 = random.randint(0, self.df0.shape[0]-499)
        n1 = random.randint(0, self.df1.shape[0]-499)
        return n0, n1
